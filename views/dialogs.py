...
def open_add_bin_dialog():
    with ui.dialog() as dialog, ui.card().classes("w-96"):
        ...
        with ui.row().classes("justify-end gap-2 mt-4"):
            ...
            ui.button("Add", on_click=lambda: [actions.add_bin_action(bid.value, btype.value, blat.value, blon.value, bfill.value), dialog.close()]).classes("bg-green-600 text-white")
    dialog.open()
...
def open_update_fill_dialog():
    with ui.dialog() as dialog, ui.card().classes("w-96"):
        ...
        with ui.row().classes("justify-end gap-2 mt-4"):
            ui.button("Cancel", on_click=dialog.close).props("flat")
            ui.button("Update", on_click=lambda: [actions.update_fill_action(bid.value, fill_val.value), dialog.close()]).classes("bg-blue-600 text-white")
    dialog.open()
...
def open_request_dialog():
    with ui.dialog() as dialog, ui.card().classes("w-96"):
        ...
        with ui.row().classes("justify-end gap-2 mt-4"):
            ui.button("Cancel", on_click=dialog.close).props("flat")
            ui.button("Submit", on_click=lambda: [actions.request_collection_action(bid.value), dialog.close()]).classes("bg-orange-500 text-white")
    dialog.open()
